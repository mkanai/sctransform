...
sample_cells_by_batch <- function(umi, n_cells, cell_attr, batch_var = NULL, batch_levels = NULL, min_cells_per_batch = 10) {
  ...
  cells_by_batch <- split(all_cells, cell_attr[all_cells, batch_var])

  # Check if any batch has no cells
  batch_sizes <- lengths(cells_by_batch)
  empty_batches <- batch_levels[batch_sizes == 0]
  if (length(empty_batches) > 0) {
    stop("Batch level(s) '", paste(empty_batches, collapse = "', '"), "' have no cells")
  }

  # Ensure the list only contains the specified batch levels and in the right order
  cells_by_batch <- cells_by_batch[batch_levels]

  # Calculate how many cells to sample from each batch
  remaining_cells <- n_cells - (length(batch_levels) * min_cells_per_batch)

  if (remaining_cells < 0) {
    stop("n_cells (", n_cells, ") is too small to sample at least ", min_cells_per_batch,
         " cells from each of the ", length(batch_levels), " batch levels")
  }
  ...
  if (cells_deficit > 0) {
    # Add the deficit to the smallest batches
    batch_order <- order(batch_sizes)
    cells_per_batch[batch_order[1:cells_deficit]] <- cells_per_batch[batch_order[1:cells_deficit]] + 1
  }
  ...
  sampled_batch_counts <- table(cell_attr[sampled_cells, batch_var])
  dropped_batch_levels <- batch_levels[!batch_levels %in% names(sampled_batch_counts)]

  if (length(dropped_batch_levels) > 0) {
    stop('Dropped batch levels ', paste(dropped_batch_levels, collapse=", "), ', implementation error')
  }
